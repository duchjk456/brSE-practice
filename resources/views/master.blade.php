<!DOCTYPE html>
<html lang="en">

<head>
    <meta charset="UTF-8">
    <meta http-equiv="X-UA-Compatible" content="IE=edge">
    <meta name="viewport" content="width=device-width, initial-scale=1, shrink-to-fit=no">
    <link href="https://fonts.googleapis.com/css?family=Roboto:300,400&display=swap" rel="stylesheet">

    <link href="https://fonts.googleapis.com/css?family=Poppins:300,400,500&display=swap" rel="stylesheet">

    <link href="https://fonts.googleapis.com/css?family=Source+Serif+Pro:400,600&display=swap" rel="stylesheet">
    <!-- Latest compiled and minified CSS -->
    <link rel="stylesheet" href="https://maxcdn.bootstrapcdn.com/bootstrap/3.4.1/css/bootstrap.min.css">

    <!-- jQuery library -->
    <script src="https://ajax.googleapis.com/ajax/libs/jquery/3.6.4/jquery.min.js"></script>

    <!-- Latest compiled JavaScript -->
    <script src="https://maxcdn.bootstrapcdn.com/bootstrap/3.4.1/js/bootstrap.min.js"></script>

    <link rel="stylesheet" type="text/css" href="{{ asset('template/06/fonts/icomoon/style.css') }}">

    <link rel="stylesheet" type="text/css" href="{{ asset('template/06/css/owl.carousel.min.css') }}">

    <link rel="stylesheet" type="text/css" href="{{ asset('template/06/css/bootstrap.min.css') }}">

    <link rel="stylesheet" type="text/css" href="{{ asset('template/06/css/style.css') }}">
    @yield('css')

    <title>Document</title>
</head>

<body>
    @yield('body')
    <script src="{{ asset('template/06/js/jquery-3.3.1.min.js') }}"></script>
    <script src="{{ asset('template/06/js/popper.min.js') }}"></script>
    <script src="{{ asset('template/06/js/bootstrap.min.js') }}"></script>
    <script src="{{ asset('template/06/js/main.js') }}"></script>
    @yield('js')
</body>

</html>